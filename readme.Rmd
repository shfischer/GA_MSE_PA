---
title: "Using a genetic algorithm to optimise a data-limited catch rule -- PA branch"
...
> Fischer, S. H., De Oliveira, J. A. A., Mumford, J. D., and Kell, L. T. (in press). Using a genetic algorithm to optimise a data-limited catch rule. ICES Journal of Marine Science. https://dx.doi.org/10.1093/icesjms/fsab018.

This is the **`PA` branch** which includes the optimisation with specific risk limits for the ICES precautionary approach (PA).
...
* `OM_sensitivity.R`, `run_ms_sensitivity.R`, and `analysis_sensitivity.R` for the sensitivity analysis (for creating the operating models, running simulations and analysing the results for pollack)
...
* `pol_PA_components_stats.csv`: exploration of including/excluding elements of the rfb-rule into the optimisation
* `all_stocks_PA_multiplier_stats.csv`: optimisation towards the PA with only the multiplier of the rfb-rule for all stocks
* `all_stocks_PA_stats.csv`: optimisation with all rfb-rule parameters and all stocks
